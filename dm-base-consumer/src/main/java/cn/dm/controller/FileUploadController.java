...
public class FileUploadController {
    ...
    @ResponseBody
    public Dto upload(HttpServletRequest request, HttpServletResponse response) throws Exception{
        ...
        while (iter.hasNext()) {
            ...
            //上传完成开始保存数据库
            fileList.add(fileName+"."+fileType);
        }
    ...
    public void download(HttpServletRequest request, HttpServletResponse response, @PathVariable("fileId") String fileId) {
        ...
        try {
            if(EmptyUtils.isEmpty(fileId)){
                printUtil.print(DtoUtil.returnFail(BaseErrorCode.File_Name_isEmpty.getErrorMessage(),BaseErrorCode.File_Name_isEmpty.getErrorCode()));;
            }
            imageFile = new File(serverSavePath+"//"+fileId);
